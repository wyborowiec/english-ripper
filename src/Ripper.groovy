...
ROOT_URL = "https://edesk.pearson.pl/Audio/"
MP3_URL = ROOT_URL + "GetAudio?access_token="
...
def getSaveFileConsumer(def fileName) {
    return {
        connection ->

            def file = new File(fileName + '.mp3')
            byte[] bytes = connection.inputStream.bytes
            println "Save audio $fileName, size: " + bytes.size()
//            println bytes
            file.bytes = bytes
    }
}
...
def parseCollectiom(def nr) {
    def xml = download("https://edesk.pearson.pl/Home/Show/$nr", {
        connection ->
            def text = connection.inputStream.text
//            println connection.responseCode + ": " + text
            text
    })
    def groups = (xml =~ /\"(\/Audio\/Index\/\d+\?p=-?\d+)\" target/)
    println groups.size()
    groups.collect {
        it[1]
    }
